...
function renderAll(ctx, canvas, avatarImg) {
    ...
    ctx.fillStyle = '#000000'; // ЧИСТЫЙ ЧЕРНЫЙ
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    const topGrad = ctx.createRadialGradient(canvas.width, 0, 50, canvas.width, 0, 400);
    topGrad.addColorStop(0, 'rgba(0, 242, 255, 0.15)'); // БИРЮЗОВЫЙ
    ...
    bottomGrad.addColorStop(0, 'rgba(0, 242, 255, 0.1)'); // БИРЮЗОВЫЙ
    ...
    particles.forEach(p => {
        ...
        g.addColorStop(1, `rgba(0, 242, 255, ${p.opacity})`); // БИРЮЗОВЫЙ
        ctx.strokeStyle = g;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(p.x, p.y);
        ...
        ctx.stroke();
        ctx.fillStyle = `rgba(0, 242, 255, ${p.opacity * 2})`; // БИРЮЗОВЫЙ
        ctx.beginPath(); ctx.arc(p.x, p.y + p.length, 1, 0, Math.PI * 2); ctx.fill();
    });

    ctx.save();
    ctx.fillStyle = "rgba(0, 242, 255, 0.03)"; // БИРЮЗОВЫЕ ТОЧКИ
    for (let x = 0; x < canvas.width; x += 30) {
        for (let y = 0; y < canvas.height; y += 30) {
            ctx.beginPath(); ctx.arc(x, y, 0.8, 0, Math.PI * 2); ctx.fill();
        }
    }
    ctx.fillStyle = "rgba(0, 242, 255, 0.04)"; // БИРЮЗОВЫЕ СИМВОЛЫ
    ...
    ctx.save();
    ctx.strokeStyle = "rgba(0, 242, 255, 0.7)"; // БИРЮЗОВАЯ РАМКА АВАТАРА
    ...
    ctx.shadowColor = "rgba(0, 242, 255, 0.6)"; // БИРЮЗОВОЕ СВЕЧЕНИЕ ЗАГОЛОВКА
    ctx.shadowBlur = 15;
    ...
    lineGrad.addColorStop(0.5, "rgba(0, 242, 255, 0.5)"); // БИРЮЗОВАЯ ЛИНИЯ
    ...
    ctx.save();
    ctx.strokeStyle = "rgba(0, 242, 255, 0.3)"; // БИРЮЗОВЫЕ ПОЛЯ
    ...
    ctx.fillStyle = "#00f2ff"; ctx.font = "18px Fredoka"; // ТЕКСТ ДАТЫ БИРЮЗОВЫЙ
    ...
    selectedRoles.forEach(role => {
        let c1, c2;
        // Цвета ролей теперь в бирюзово-белой гамме
    ...
    ctx.strokeStyle = "rgba(0, 242, 255, 0.3)"; // БИРЮЗОВОЕ ПОЛЕ БИО
    ...
    drawIcon(185, sY, "#00f2ff", 'x'); ctx.fillText("Twitter", 207, sY); // ИКОНКИ БИРЮЗОВЫЕ
    ...
    kastGrad.addColorStop(0, "#ffffff"); kastGrad.addColorStop(1, "#00f2ff"); // ЛОГО БЕЛО-БИРЮЗОВОЕ
    ...
    if (isGenerating) {
        ...
        scanGrad.addColorStop(1, "rgba(0, 242, 255, 0.4)"); // СКАНЕР БИРЮЗОВЫЙ
        ctx.fillStyle = scanGrad;
        ctx.fillRect(0, scanLineY - 40, canvas.width, 40);
        ctx.strokeStyle = "#00f2ff"; // ЛИНИЯ СКАНЕРА БИРЮЗОВАЯ
        ctx.lineWidth = 2;
        ctx.shadowBlur = 15;
...
(function() {
    ...
    function animate() {
        bgCtx.fillStyle = '#000000'; // ФОН СТРАНИЦЫ ЧЕРНЫЙ
        ...
        bgLines.forEach(l => {
            ...
            g.addColorStop(0, 'transparent');
            g.addColorStop(1, `rgba(0, 242, 255, ${l.op})`); // ЛИНИИ ФОНА БИРЮЗОВЫЕ
            bgCtx.strokeStyle = g;
            bgCtx.lineWidth = 1.2;
            bgCtx.beginPath(); 
